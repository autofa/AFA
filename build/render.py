...
class MarkdownWithMath(mistune.Markdown):
    ...
    def output_block_math(self):
        return self.renderer.block_math(self.token['text'])  # type: ignore
...
class PythonDocxRenderer(mistune.Renderer):
    ...
    def table(self, header, body):
        ...
        cells = ["table.rows[%d].cells[%d].paragraphs[0]%s\n" % (i, j, self.table_memory.pop(
            0)[1:]) for i, j in itertools.product(range(number_rows), range(number_cols))]
